...
class TestBenchmarking(unittest.TestCase):
    ...
    def test_compare_cluster_entity(self):
        ...
        # self.assertTrue(results['percentage_correct_entity_type'], 1)

        entity_1 = [{'id': 0, 'type': 'cluster', 'name': 'payed parking spot', 'minPoints': 3, 'maxDistance': '50 m'},
         {'id': 1, 'name': 'church', 'type': 'nwr'}, {'id': 2, 'name': 'tower', 'type': 'cluster', 'properties': [
            {'name': 'height', 'operator': '=', 'value': '100'}]}]
        ...
        print('percentage entities')
        print(results['percentage_correct_entity_type'])
        self.assertAlmostEqual(results['percentage_correct_entity_type'], 0.667, places=3)

        # _, paired_entities, unpaired_entities = self.entity_analyzer.pair_entities(entity_1, entity_2)
        ...
        # assert len(unpaired_entities['ground_truth']) == 0
