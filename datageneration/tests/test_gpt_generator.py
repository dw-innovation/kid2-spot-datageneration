...
class TestGPTGenerator(unittest.TestCase):
    ...
    def test_add_relation_with_contain(self):
        class MockedPromptHelper(PromptHelper):
            def shuffle_and_select_phrase(self, candidate_phrases):
                return "within"

        # test case 1
        test_rels = [Relation(type='contains', source=0, target=1, value=None)]
        class_instance = MockedPromptHelper(relative_spatial_terms=self.gen.relative_spatial_terms)

        expected_prompt = 'Obj. 1 is within Obj. 0\n'
        generated_prompt, _ = class_instance.add_relation_with_contain(test_rels)
        self.assertTrue(generated_prompt == expected_prompt)
        ...
        # test case 3
        test_rels = [Relation(type='contains', source=2, target=0, value=None),
                     Relation(type='dist', source=2, target=1, value='780.8 in'),
                     Relation(type='dist', source=0, target=1, value='780.8 in'),
                     Relation(type='dist', source=2, target=3, value='5 yd'),
                     Relation(type='dist', source=0, target=3, value='5 yd')]

        expected_prompt = 'Obj. 0 is within Obj. 2\n'
        generated_prompt, individual_rels = class_instance.add_relation_with_contain(test_rels)
        self.assertTrue(generated_prompt == expected_prompt)
